...
def train():
    ...
    for epoch in range(wandb.config['num_train_epochs']):
        ...
        for step, (enc_inputs, dec_inputs, target_batch) in enumerate(eng2kor_dataloader):
            ...
            wandb_loss[-1].append(loss.item())
        for loss in wandb_loss:
            wandb.log({'kor2eng_loss': loss[0], 'eng2kor_loss': loss[1], 'combined_loss': loss[0] + loss[1]})
